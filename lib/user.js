...
class User {
  ...
  static async create(firstName, lastName, email, password, dob, profilePic) {
    ...
    if (await User.checkUserExists(email)) {
      ...
      await User.addPhoto(profilePic)
  ...
  static async addPhoto(photoFile) {
    console.log(photoFile);
    let bytes = new Buffer(fs.readFileSync(photoFile.path)).toString("base64");
    console.log(bytes);
  }
