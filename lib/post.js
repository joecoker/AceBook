...
class Post {
  ...
  static async list(userId) {
    let postList = await dbc.query(`
      ...
      COALESCE((SELECT 'TRUE'::TEXT FROM likes WHERE posts.postid = likes.postid AND likes.userid = ${userId}), 'FALSE'::TEXT)::BOOLEAN as liked
      FROM posts
...
static async getUserPosts(userId, posts = 10) {
  ...
  static async getPost(postId, userId){
    let result = await dbc.query(`
      SELECT content, posts.postid, posts.createdat, firstname, lastname, posts.userid,
      COUNT(DISTINCT likeid)::INTEGER AS likecount,
      COALESCE((SELECT 'TRUE'::TEXT FROM likes WHERE posts.postid = likes.postid AND likes.userid = ${userId}), 'FALSE'::TEXT)::BOOLEAN as liked
      FROM posts
      LEFT JOIN likes on posts.postid = likes.postid
