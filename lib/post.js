...
class Post {
  ...
  static async list() {
    let postList = await dbc.query(`
      SELECT posts.content::TEXT, posts.createdat, firstname::TEXT, lastname::TEXT, posts.postid,
      COUNT(DISTINCT commentid)::INTEGER AS commentcount,
      COUNT(DISTINCT likeid)::INTEGER AS likecount,
      COALESCE((SELECT 'TRUE' FROM likes WHERE posts.postid = likes.postid AND likes.userid = 1), 'FALSE')::BOOLEAN as liked
      FROM posts
