...
app.post('/newsfeed', async function(req, res) {
  ...
  await Post.create(postContent, req.session.userId);
  res.redirect('/newsfeed')
})
...
app.get('/post/like/:postid', async function(req, res){
  ...
  let postId = req.params.postid;
  await Like.toggleLike(postId, req.session.userId);
...
app.post('/comment/:postid', async function(req, res){
  ...
  await Comment.create(commentContent, postId, req.session.userId);
...
app.post('/user/update/:userid', async function(req, res){
  let userDetails = await User.updateProfile(
    req.params.userid, 
    req.body.firstName, 
    req.body.lastName, 
    req.body.email, 
